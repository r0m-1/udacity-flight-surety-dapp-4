...
contract FlightSuretyApp {
    ...
    */
    ...
    // hash(index, flight, timestamp) => status code reported => addresses
    mapping(bytes32 => mapping(uint8 => address[])) private oracleResponseStatusAddresses;
    ...
    external
    {
        ...
        oracleResponseStatusAddresses[key][statusCode].push(msg.sender);

        // Information isn't considered verified until at least MIN_RESPONSES
        // oracles respond with the *** same *** information
        emit OracleReport(airline, flight, timestamp, statusCode);
        if (oracleResponseStatusAddresses[key][statusCode].length >= MIN_RESPONSES) {
